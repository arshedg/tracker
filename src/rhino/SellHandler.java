...
public class SellHandler extends EquityProcessor implements Runnable {
    ...
    // if(!isBadMood()) return;
        ...
        this.purchasePrice=bestBuy;//getLastPrice();
...
//        }
    ...
    private boolean isTimeOut() {
        if (isDebugging) {
            if (parser.getCursor() - cursor > 30) {
                return true;
            }
        } else {
            if (priceDetails.size() - cursor > 30) {
                return true;
            }
        }
        return false;
    ...
    private void debugGainOrLoss(float target, float stopLoss) {
        ...
        for (int i = cursor; i < datas.size(); i++) {
            ...
            if (value >= target) {
                ...
                swicthOFF = true;
//               BuyHandler handler= new BuyHandler(name, value,process);
//               handler.priceDetails=this.priceDetails;
//               handler.process();
                return;
            }
        }
    ...
    private float isReadyToBuy(boolean recheck, float triggeredPrice) {
        ...
        float highest=currentPrice;
        while (negligiblyEqualorLesser(currentPrice, highest)) {
            try {
